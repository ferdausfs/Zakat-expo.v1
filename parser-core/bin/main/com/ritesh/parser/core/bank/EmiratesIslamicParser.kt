...
class EmiratesIslamicParser : BankParser() {
    ...
    override fun canHandle(sender: String): Boolean {
        ...
        return upperSender.contains("EMIRATESISLAMIC") || upperSender.contains("EIB")
    }

    override fun extractAmount(message: String): BigDecimal? {
        val patterns = listOf(
            Regex("""AED\s+([0-9,]+(?:\.[0-9]{1,2})?)""", RegexOption.IGNORE_CASE),
            Regex("""aed\s+([0-9,]+(?:\.[0-9]{1,2})?)""", RegexOption.IGNORE_CASE)
        )
